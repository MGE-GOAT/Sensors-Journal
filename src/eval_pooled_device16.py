...
MODELS_DIR = os.path.expanduser(os.environ.get("MODELS_DIR", "~/wuwexp/models/ladder"))
...
def main():
    ...
    for pos_dir in all_positions:
        ...
        for cls in ["wake", "speech", "noise"]:
            ...
            if not os.path.exists(wav_path):
                ...
                feats = None
                break
            a = load_ch0_raw(wav_path) * 16.0  # EXACT device scaling (S32*1/2^27 reconstructed from S16 capture)
    ...
    out = {"pipeline": "canonical_no_normalization",
           ...
           "pvalues": pvals}
    out_path = os.path.expanduser(os.environ.get("OUT_PATH", "~/wuwexp/domainshift/pooled_significance_results_DEVICE_GAIN16.json"))
